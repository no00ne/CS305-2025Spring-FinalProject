...
Taking Ethereum as an example, it is a public blockchain system which all entities can freely join or leave. The entities in the system are called blockchain peers (*simply* **peers**), which can not only generate transactions but also verify transactions generated by other peers and package valid transactions in the form of blocks. These blocks are then verified by all peers in the system and linked to the blockchain if most peers accept them. This is called the blockchain consensus (i.e., block generation and verification). Here, each peer stores a copy of the blockchain locally. Thus, a block is linked to the blockchain means that each peer stores the block in their local blockchains.

The simple operation procedure of a blockchain system is as follows:

1. Peers generate new transactions and broadcast them to all peers in the system.
   
2. Peers receiving the transactions verify their validity. If the transactions are valid, peers add them to their local transaction pools.
   
3. In each block period (i.e., the interval to generate a new block), one of the peers is selected as a block generator to package transactions in its pool as a block and broadcast the block to all peers in the system for verification.
   
4. If most peers accept the block, each peer appends the block to the blockchain.
