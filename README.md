...
After the containers are running, execute `./demo.sh` to demonstrate the network. The script performs a health check on each dashboard, prints the initialization logs, submits a sample transaction through `/transactions/new`, waits for the block to be mined, and then queries metrics such as `/latency`, `/capacity`, and `/blacklist` from ports 8000‑8010.
