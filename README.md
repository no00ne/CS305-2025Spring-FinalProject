...
### Part 3: Sending Messages (outbox.py)
...
* Select the best relaying peer


8. `relay_or_direct_send` (`network/link_simulator.py`)
...
9. `get_relay_peer` (`network/link_simulator.py`)

* Find the set of relaying candidates reachable from the target peer in `reachable_by` of `peer_discovery.py`.
    
* Read the transmission latency between the sender and relaying candidate in `rtt_tracker` of `peer_manager.py`.
  
* Return the best relaying peer with the smallest transmission latency.

10. `send_message`

* Send the message to the target peer. Wrap the function `send_message` with the dynamic network condition in the function `apply_network_condition` of `link_simulator.py`.
